...
async fn dhcp_server_task(stack: Stack<'static>) -> ! {
    ...
    loop {
        let Ok((len, _src)) = socket.recv_from(&mut in_buf).await else {
...
async fn http_client_task(stack: Stack<'static>) -> ! {
    ...
    loop {
        ...
        let endpoint = IpEndpoint::new(IpAddress::Ipv4(Ipv4Address::new(172, 31, 1, 2)), 8080);
        ...
        if let Err(e) = socket.write_all(req).await {
            warn!("http write error: {:?}", e);
            continue;
        }

        match socket.read(&mut response).await {
            Ok(0) => warn!("http response EOF"),
            Ok(n) => info!("http response ({} bytes): {:a}", n, &response[..n]),
            Err(e) => warn!("http read error: {:?}", e),
        }
    }
}

/// フルサンプル: CDC-NCM + Pico 側固定 IP + DHCP で 172.31.1.2 を払い出し + 172.31.1.2:8080 へ HTTP GET します。
...
async fn main(spawner: Spawner) {
    ...
    spawner.spawn(http_client_task(stack).unwrap());
